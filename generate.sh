...
curl -s --connect-timeout 5 --retry 5 --cacert riseup-ca.crt https://api.black.riseup.net/3/cert -o riseup-vpn.pem
...
IP_LIST=$(curl -s --connect-timeout 5 --retry 5 --cacert riseup-ca.crt https://api.black.riseup.net/3/config/eip-service.json | jq -r .gateways[].ip_address)
